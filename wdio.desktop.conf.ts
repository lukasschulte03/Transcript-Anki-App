...
const qaOutput = process.env.LECTIO_QA_OUTPUT_DIR
  ? path.resolve(process.env.LECTIO_QA_OUTPUT_DIR)
  : path.resolve("test-results/desktop");
const stabilityRoot = process.env.LECTIO_STABILITY_ROOT
  ? path.resolve(process.env.LECTIO_STABILITY_ROOT)
  : qaOutput;
const isolatedAppData = path.join(stabilityRoot, "appdata");
  ...
  services: [
    [
      "@wdio/tauri-service",
      {
        ...
        logDir: path.join(qaOutput, "desktop", "logs"),
        startTimeout: 90_000,
